    ...
    if page_args.isdigit() == True:
        ...
        if page >= 0 and page <= lengthData//12:
            ...
            if keyword == None:
                ...
                start = page*12
                cursor.execute(
                    f"SELECT * FROM taipei_attractions LIMIT {start},12")
            ...
            # 使用keyword查詢!
                ...
                cursor.execute(
                    f'SELECT * FROM taipei_attractions WHERE name LIKE "%{keyword}%" OR category LIKE "%{keyword}%" OR description LIKE "%{keyword}%" OR address LIKE "%{keyword}%" OR mrt LIKE "%{keyword}%" LIMIT {begin},{end}')
                resultall = cursor.fetchall()
                for resultData in resultall:
                    result(resultData)
                # nextPage(page)
...
def getAttBYid(attractionId):
    ...
    if attractionId.isdigit() == True:  # 判定id是否為數字
        ...
        if serchId > 0 and serchId <= lengthData:  # id介於有效範圍
            cursor.execute(
                f"SELECT * FROM taipei_attractions WHERE id={serchId}")
